...
### The GenericForeignKey Puzzle
This one took me a minute. I needed notifications for different things - someone follows you, likes your post, comments, etc. But each notification points to a different type of thing (a User for follows, a Post for likes).
...
### Deploying Without CLI Tools
Tried to deploy to Heroku but didn't have their CLI installed. Instead of installing it, I just figured out how to do everything through their web dashboard. Actually wasn't that hard once I found the right buttons to click.
...
As I added more features, my test file got messy fast. Tests were all over the place and some were basically duplicates.
...
Hardcoding database passwords and secret keys is obviously bad. I knew that but still had to figure out the right way to handle it.
...
**More Features**
Thinking about adding real-time notifications with WebSockets. Also direct messaging would be cool. Maybe image uploads for posts and profiles.
...
Time management was rough. Deployment took way longer than I thought it would. Should have budgeted more time for things I haven't done before.
...
Django's documentation is actually really good. Whenever I was stuck, usually found the answer there. Just had to actually read it instead of skimming.
